...
def run_recording_generation(
    ...
    *,
    ...
    """Run the save_chatbot_response_recordings.py script with given config."""
    cmd = [
        "uv",
        "run",
        "./scripts/save_chatbot_response_recordings.py",
    ...
    logger.info(f"Generating {config.description} for {voice_name} voice")
...
def main(remove_silence: bool, skip_fillers: bool, only_fillers: bool, voice_name: str | None) -> None:
    """Generate all chatbot response recordings for all configured voices."""
    ...
    logger.info(f"Starting generation of {total_count} recording sets for voices: {voice_names}")
