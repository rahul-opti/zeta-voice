...
def _patch_settings_for_dynamics(monkeypatch: pytest.MonkeyPatch) -> None:
    """Fixture to enable Dynamics ERP booking for the duration of a test."""
    monkeypatch.setattr(settings.calendar, "DYNAMICS_ERP_BOOKING", True)
...
async def test_conversation_with_mocked_llm_dynamics(
    ...
    """
    Test the conversation runner with Dynamics enabled and both LLM and Dynamics API mocked.
    """
    ...
    with (
        ...
        start_conversation_request = StartCallRequest(
            to_number="test_number", lead_id="test_lead", handoff_number="test_number"
        )
        ...
        initial_message = await runner.start_conversation(call_sid="test_call_sid")
